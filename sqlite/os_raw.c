...
struct rawFile {
  ...
  FIL * h;                            /* Pointer to access the file */
...
static int rawRead(
  ...
  /* seek pos */
  rc = f_lseek(pFile->h, (DWORD)offset);
  if( rc != FR_OK ){
  ...
  /* do read file */
  rc = f_read(pFile->h, pBuf, amt, &got);
  if( rc != FR_OK ){
...
static int rawWrite(
  ...
  /* do write file */
  rc = f_write(pFile->h, pBuf, amt, &wrote);
  if( rc != FR_OK ){
...
static int rawTruncate(sqlite3_file *id, sqlite3_int64 nByte){
  ...
  */
  ...
  rc = f_lseek(pFile->h, (DWORD)nByte);
  if( rc != FR_OK ){
    return SQLITE_IOERR_TRUNCATE;
  }

  rc = f_truncate(pFile->h);
  if( rc != FR_OK ){
    return SQLITE_IOERR_TRUNCATE;
  }
